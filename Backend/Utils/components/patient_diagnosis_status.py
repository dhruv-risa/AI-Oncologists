...
def extract_diagnosis_status_with_gemini(pdf_input):
    ...
    "8. DISEASE STATUS: Extract current disease status using INDUSTRY-STANDARD ONCOLOGY TERMINOLOGY.\n"
    "   Look for clinician assessments of disease behavior and treatment response.\n"
    "   Use these STANDARD TERMS based on what is documented:\n"
    "   - Response Categories (RECIST-based):\n"
    ...
    "     * 'Progressive Disease (PD)' - Tumor growth or new lesions\n"
    "   - Clinical Status Terms:\n"
    "     * 'Newly Diagnosed' - Recent diagnosis, pre-treatment\n"
    "     * 'Active Disease' - Ongoing cancer requiring treatment\n"
    "     * 'Responding to Treatment' - Positive response to therapy\n"
    "     * 'Recurrent Disease' - Cancer returned after remission\n"
    "     * 'Remission' or 'No Evidence of Disease (NED)' - No detectable cancer\n"
    "   Map clinician's language to these standard terms. If the clinician writes 'responding well',\n"
    "   use 'Responding to Treatment'. If they write 'progression', use 'Progressive Disease (PD)'.\n"
    "   ⚠️ Return null ONLY if no disease status information is documented.\n\n"
    ...
    "- disease_status MUST use industry-standard oncology terminology\n"
    ...
    "disease_status": (
        "Current disease status using STANDARD ONCOLOGY TERMINOLOGY. "
        "REQUIRED: Use one of these industry-standard terms based on documented information:\n"
        "  RESPONSE CATEGORIES (RECIST-based):\n"
        "    - 'Complete Response (CR)' - No evidence of disease, complete tumor disappearance\n"
        "    - 'Partial Response (PR)' - ≥30% decrease in tumor size, significant improvement\n"
        "    - 'Stable Disease (SD)' - No significant change, neither growth nor shrinkage\n"
        "    - 'Progressive Disease (PD)' - ≥20% increase in tumor size, new lesions, or worsening\n"
        "  CLINICAL STATUS:\n"
        "    - 'Newly Diagnosed' - Recently diagnosed, treatment planning stage\n"
        "    - 'Active Disease' - Ongoing cancer requiring active treatment\n"
        "    - 'Responding to Treatment' - Positive response to current therapy\n"
        "    - 'Recurrent Disease' - Cancer has returned after remission\n"
        "    - 'Remission' or 'No Evidence of Disease (NED)' - No detectable cancer\n"
        "IMPORTANT: Map clinician's language to these standard terms:\n"
        "  - 'stable' → 'Stable Disease (SD)'\n"
        "  - 'progression', 'progressing', 'worsening' → 'Progressive Disease (PD)'\n"
        "  - 'responding', 'improved', 'better' → 'Responding to Treatment' or 'Partial Response (PR)'\n"
        "  - 'complete response', 'no disease', 'NED' → 'Complete Response (CR)'\n"
        "  - 'recurrence', 'recurrent' → 'Recurrent Disease'\n"
        "⚠️ Return null ONLY if absolutely no disease status information is available."
    )
}
